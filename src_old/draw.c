...
/*   Updated: 2025/01/11 18:37:48 by ybouryal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_line(t_fdf *vars, t_point *p1, t_point *p2)
{
	int	dx;
	int	dy;
	int	sx;
	...
	while (y < vars->rows)
	{
		...
		while (x < vars->cols)
		{
			...
			if (curr->next && x < vars->cols)
				draw_line(vars, project(scale(vars, curr, x, y)) + vars->offset_x, project(scale(vars, curr->next, x + 1, y)) + vars->offset_y);
			...
			if (next_row && y < vars->rows)
			{
				...
				if (under)
				{
					...
					draw_line(vars, project(scale(vars, curr, x, y)) + vars->offset_x, project(scale(vars, under, x, y + 1)) + vars->offset_y);
					// draw_line(vars, project(scale(vars, curr, x, y)), project(scale(vars, under, x, y + 1)));
					// draw_line(vars, curr, under);
